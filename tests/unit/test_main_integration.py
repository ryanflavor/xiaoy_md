...
class TestMainIntegration:
    ...
    async def test_service_initialization_with_nats(self, test_settings):
        ...
        with patch("src.__main__.settings", test_settings):
            with patch("src.__main__.NATSPublisher") as mock_publisher_class:
                with patch("src.__main__.MarketDataService") as mock_service_class:
        ...
        with patch("src.__main__.settings", test_settings):
            with patch("src.__main__.NATSPublisher") as mock_publisher_class:
                with patch("src.__main__.MarketDataService") as mock_service_class:
                    ...
                    def track_signal_handler(sig, handler, *args):
    ...
    def test_main_success(self, test_settings):
        ...
        with patch("src.__main__.settings", test_settings):
            with patch("src.__main__.setup_logging") as mock_setup_logging:
    ...
    def test_main_keyboard_interrupt(self, test_settings):
        ...
        with patch("src.__main__.settings", test_settings):
            with patch("src.__main__.setup_logging"):
                with patch("asyncio.run") as mock_async_run:
