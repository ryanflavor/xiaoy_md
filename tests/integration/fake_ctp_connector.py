...
def set_on_tick(callback: Any) -> None:  # production API expected by src.main
    ...
    gateway_connect._on_tick = callback  # type: ignore[attr-defined]
...
def gateway_connect(setting: dict[str, object], should_shutdown: "Callable[[], bool]") -> None:
    """Emit one tick and return to let the session end cleanly."""
    cb = getattr(gateway_connect, "_on_tick", None)
    if cb is None:
        return
